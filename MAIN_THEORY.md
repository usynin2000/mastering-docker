CHAT: https://chatgpt.com/c/692154fb-2b80-8329-aef2-96050f05f587 ouc

> Контейнер — это способ изолировать приложение и его зависимости так, чтобы оно работало одинаково в разных средах. 

> Технически контейнер — это запущенный экземпляр образа (image).

Контейнер получает:

- собственное пространство имён процессов (PID),
- отдельные сетевые неймспейсы (network),
- файловую систему (комбинация read-only слоёв образа + writable слой сверху),
- ограничение ресурсов через cgroups (CPU, память, I/O).


Ключевые идеи:
- Легковесность: контейнеры делят ядро ОС с хостом, поэтому они намного « легче » виртуальных машин.
- Повторяемость: контейнер запускает приложение в предсказуемой среде.
- Скорость старта: контейнеры запускаются почти мгновенно.

> Аналогия: образ — это рецепт + готовые ингредиенты; контейнер — это приготовленное блюдо, которое можно есть (использовать).


### Контейнер vs виртуальная машина (VM)

> Коротко — у контейнера и VM похожие цели (изоляция), но разные уровни изоляции и накладные расходы.


| Параметр           |                                         Контейнер | Виртуальная машина                                 |
| ------------------ | ------------------------------------------------: | -------------------------------------------------- |
| Изоляция           |        Процессы, неймспейсы, cgroups (делит ядро) | Полная — свое ядро, BIOS, аппаратная виртуализация |
| Размер             |                  Мегабайты — образы обычно меньше | Гигабайты — целая ОС                               |
| Время старта       |                            Миллисекунды — секунды | Минуты                                             |
| Производительность |        Почти нативная (меньше накладных расходов) | Накладные расходы на виртуализацию                 |
| Использование      | Микросервисы, CI, dev, контейнеризация приложений | Изоляция разных ОС, старое ПО, гипервизоры         |


Когда выбирать что:

Нужна быстрая масштабируемость и консистентность среды — контейнеры.

Требуется полная аппаратная/ядровая изоляция или запуск другой ОС — VM.


### Docker Engine, Daemon, CLI — что это и как они связаны
- Docker Engine — это общее название платформы Docker, которая включает в себя серверную часть и клиент.

- Компоненты: Docker Daemon (server), REST API, Docker CLI (client).

- Docker Daemon (dockerd) — фоновый процесс, который управляет образами, контейнерами, сетью и томами. С ним взаимодействует CLI через API. На Linux обычно dockerd.

- Docker CLI (docker) — командная утилита, которую ты запускаешь в терминале (docker run, docker build и т.д.). CLI отправляет команды на daemon через REST API (локально по сокету либо по TCP).

Пример взаимодействия:
Ты набираешь docker run nginx. 

CLI формирует запрос -> отправляет daemon -> daemon создаёт/запускает контейнер -> возвращает статус.


### Основные понятия: Image, Container, Registry, Layers, Tag

#### Image (образ)
- Образ — это слепок файловой системы и метаданных, который служит шаблоном для запуска контейнеров.
- Образы обычно создаются из Dockerfile и состоят из слоёв (layers). Образы неизменяемы (immutable).
- Команды:
    - docker images — список локальных образов.
    - docker pull ubuntu:22.04 — скачивает образ из реестра.

Пример простого Dockerfile:
```dockerfile
FROM python:3.11-slim
WORKDIR /app
COPY requirements.txt .
RUN pip install -r requirements.txt
COPY . .
CMD ["python", "app.py"]
```

docker build -t myapp:1.0 .


#### Container (контейнер)

- Контейнер — это запущенный (или созданный, но не запущенный) экземпляр образа. Он добавляет сверху образа writable layer, где происходят изменения во время работы.

- Writable layer — что это такое?
- Когда ты запускаешь контейнер из образа:
    - образ состоит из набора слоёв (layers) — они только чтение
    - контейнер добавляет верхний слой, который можно изменять → writable layer
📚 Представь образ как “застывшую копию файловой системы”

Например, образ Ubuntu.
Он содержит:
- /bin
- /usr
- /etc
- /var
- файлы библиотек
- конфиги
- всё это уже “запаковано” в layers

Образ неизменяемый (immutable).
- Когда ты запускаешь контейнер, Docker кладёт на вершину ещё один слой → writeable layer.

✍ В writable layer попадают:
- новые файлы
- изменения существующих файлов
- удалённые файлы
- лог-файлы
- кэш и временные данные

♻ Когда контейнер удаляется — writable layer исчезает. Именно поэтому контейнеры "эфемерные".

- Команды:
```shell
docker run -d --name web -p 8080:80 nginx  # — запустить nginx в фоне, пробросив порт.
docker ps # — список запущенных контейнеров.
docker ps -a # — все контейнеры, включая остановленные.
docker exec -it <container> /bin/bash # — зайти внутрь запущенного контейнера.
docker stop <container> / docker rm <container>.
```

Важно: если удалить контейнер — writable слой и данные в нём удаляются (если не использованы volume).


### Registry (реестр)

- Registry — это сервис для хранения и распространения образов. Примеры: Docker Hub, GitHub Container Registry, GitLab Registry, приватные реестры (Harbor, Artifactory).

- Образы пушатся/пуллятся по имени registry.domain.com/namespace/image:tag.

Команды:
    - docker push myrepo/myimage:1.0
    - docker pull myrepo/myimage:1.0
    - docker login — аутентификация.


### Layers (слои)

- Слои — это цепочка файловых изменений (diff), где каждый слой создаётся командой в Dockerfile (например RUN, COPY, ADD).
- Слои являются только для чтения (read-only) и объединяются с помощью union filesystem (overlay2 и т.п.). Последний слой — writable для контейнера.
- Преимущества:
    - Кэширование при сборке: если слой не поменялся, Docker берёт его из кеша.
    - Повторное использование: слои могут разделяться между образами, уменьшая объём хранимых данных.
- Команды для просмотра:
    - docker history myimage:tag — список слоёв и размеров.
    - docker inspect myimage:tag — метаданные (включая слои, rootfs).

Практическое правило: минимизируй количество больших слоёв и используй .dockerignore, старайся объединять команды RUN там, где это разумно, чтобы уменьшить число слоёв и размер.


### Tag (тег)
- Тег — читабельный алиас для образа (обычно указывает версию). Формат: image:tag. По умолчанию тег latest.
- Тег указывает на определный manifest / digest в реестре. Тег можно перетегировать локально:
    - docker tag myapp:1.0 myrepo/myapp:1.0
    - docker push myrepo/myapp:1.0
- Digest — хеш (sha256) образа, уникально определяет конкретный набор слоёв; тег может указывать на digest, но тег сам по себе изменяем (можно переместить тег на другой digest).
Digest однозначно идентифицирует конкретный набор слоёв образа.
Если тег можно изменить, то digest — нет. Тег — это “человеческая метка”. Digest — это “истинный уникальный идентификатор”.

### Volume (Том)
- **Volume (том)** — это отдельная область хранения данных, которую Docker создаёт и управляет вне файловой системы контейнера.
🧠 Почему вообще появились volumes?

- Контейнеры эфемерны — они живут временно.
- Если контейнер удалить — все данные внутри него исчезают.
- Но реальные приложения обычно требуют персистентное, то есть постоянное хранение данных:
    - базы данных
    - логи
    - загруженные пользователями файлы
    - кеши
    - настройки

Поэтому Docker придумал механизм volumes, позволяющий сохранять данные независимо от контейнеров.

##### 📦 Как устроен volume?
Volume — это:

- каталог на хост-машине
- который Docker сам создаёт и поддерживает
- и который может быть примонтирован в один или несколько контейнеров



🧩 Где volumes живут?
По умолчанию Docker хранит volumes здесь:
```shell
/var/lib/docker/volumes/<volume_name>/_data/
```
Ты обычно не работаешь с этим вручную — Docker сам управляет этим местом.

🏗 Типы volume
1. Named volume (именованный)
    Создаётся Docker сам:
```shell
docker volume create mydata
```
И используешь:
```shell
-v mydata:/data
```

2. Anonymous volume (анонимный)
Создаётся автоматически, если указать путь без имени:
```shell
docker run -v /data ...
```
Docker сам создаст volume с рандомным названием.

3. Bind mount
Привязываешь конкретную папку хоста:
```shell
-v /home/user/myfolder:/data
```
Это даёт полный контроль, но:
- может быть небезопасно
- данные зависят от структуры хоста


### Практическая демонстрация (шаги и команды для видео)
Предложу короткий сценарий с набором команд, которые хорошо показать в видео:

1. Скачиваем образ и запускаем контейнер:
```shell
docker pull nginx:alpine

docker run --name demo-nginx -d -p 8080:80 nginx:alpine
```

Покажи docker ps, зайди в браузер на localhost:8080 (или покажи curl):
```shell
curl http://localhost:8080
```

2. Исследуем образ и контейнер:
```shell
docker images
docker inspect demo-nginx
docker logs demo-nginx
docker exec -it demo-nginx sh
```

3. Показать историю слоёв:
```shell
docker history nginx:alpine
```
Эта команда показывает все слои образа, в том виде, как они были созданы командами Dockerfile.

И ты увидел примерно 20 слоёв.

Это абсолютно нормально: официальный образ nginx — сложный, с кучей внутренних оптимизаций.

**Как читать вывод docker history**
```shell
b3c656d55d7a   3 weeks ago   RUN /bin/sh -c ...   42.6MB
```

Это означает:

- b3c656d55d7a — ID слоя (hash)
- 3 weeks ago — когда слой был создан
- RUN /bin/sh -c … — какая команда создала слой в Dockerfile (RUN, COPY, CMD, ENV и т.д.)
- 42.6MB — размер слоя

4. Собираем собственный образ:
- Создай свой Dockerfile:
```dockerfile
FROM alpine:3.18

RUN echo "Hello from Docker!" > /hello.txt

CMD ["cat", "/hello.txt"]

```

Собери:
```shell
docker build -t mytest:1.0 .
```

Запусти:
```shell
docker run mytest:1.0
```

Посмотри историю:
```shell
docker history mytest:1.0
```

Ты увидишь, что Dockerfile превратился в 2–3 слоя:
- слой Alpine
- слой RUN echo ...
- слой с инструкцией CMD (вес 0B)

Так ты поймёшь связь:
**каждая команда Dockerfile → новый слой.**


5. Собираем собственный образ c кодом:

Создай:
    app.py:

```python
print("Hello from Python inside Docker!")
```

Dockerfile:
```dockerfile
FROM python:3.11-slim
WORKDIR /app
COPY app.py .
CMD ["python", "app.py"]
```

Собери:
```shell
docker build -t mypython:1.0 .
```

Запусти:
```shell
docker run mypython:1.0
```

6. Теги и пуш в реестр (демо с Docker Hub):
```shell
docker login -u printfchamp
# Вводим пароль
docker tag mytest:1.0 printfchamp/mytest:1.0
docker push printfchamp/mytest:1.0
```
✔️ создал собственный образ
✔️ пометил его тегом (tag)
✔️ загрузил его (push) в удалённый реестр — Docker Hub


Когда ты выполнил:
```shell
docker tag mytest:1.0 printfchamp/mytest:1.0
docker push printfchamp/mytest:1.0
```

Docker сделал следующее:

1. Взял локальный образ mytest:1.0

2. Переименовал его в формат реестра:
```shell
docker.io/printfchamp/mytest:1.0
```

3. Разделил образ на слои
4. Отправил эти слои в Docker Hub, если:
 - такого слоя нет в облаке → он "Pushed"
 - слой уже есть (например, если это базовый python:3.11) → его не отправляют (экономия)
5. В конце Docker Hub создал манифест, который указывает какие слои составляют твой образ.
```shell
s.usynin•coding/mastering-docker/python_app(master⚡)» docker push printfchamp/mytest:1.0
The push refers to repository [docker.io/printfchamp/mytest]
21cb32756ffa: Pushed 
69d8fe2b4477: Pushed 
44cf07d57ee4: Pushed 
1.0: digest: sha256:3fad35855e357c0682785f8dcab4e777736d84456b0b2a5ed4f7befefa97d224 size: 855
```

И ты увидел это:
```shell
digest: sha256:3fad35855e357c0682785f8dcab4e777736d84456b0b2a5ed4f7befefa97d224
```
Это уникальный хэш твоего образа — гарантия, что он неизменён.


### 🎯 Зачем вообще нужен push в Docker Hub

У образов есть три основные цели.

1) Хранение образов вне твоего локального компьютера

Если ноутбук сломается — всё равно безопасно.
> Docker Hub → как GitHub, но для образов.

2) Запуск образа где угодно
- Теперь любой человек (или сервер) может сделать:
```shell
docker pull printfchamp/mytest:1.0
docker run printfchamp/mytest:1.0
```
Это значит:
- у тебя появляется единый артефакт сборки
- приложение работает одинаково на всех машинах


3) Использование в CI/CD

Реальный сценарий:

1. Ты коммитишь в GitHub
2. GitHub Actions собирает образ
3. Пушит в Docker Hub
4. Сервер (например, VPS, Kubernetes) делает docker pull и обновляется

Это стандартный современный workflow.

4) Возможность разворачивать сервисы на серверах без установки зависимостей

Представь, что твой mytest — это реальное приложение.

Без Docker:
- на сервере должен быть установлен Python
- нужная версия пакетов
- нужная версия system libs и т.д


С Docker:
- на сервере вообще ничего не нужно, кроме Docker
- всё уже внутри образа

Контейнер = приложение + его окружение.

#### Где теперь можно использовать твой образ?
✔️ Локально на другой машине
```shell
docker pull printfchamp/mytest:1.0
docker run printfchamp/mytest:1.0
```

✔️ На VPS / сервере

(например, DigitalOcean или Hetzner)
```shell
docker run -d printfchamp/mytest:1.0
```

✔️ В Kubernetes

В манифесте Deployment:
```yaml
containers:
  - name: mytest
    image: printfchamp/mytest:1.0
```

✔️ В Docker Compose
```yaml
services:
  app:
    image: printfchamp/mytest:1.0
```

✔️ У других людей
Твой образ можно публиковать как часть обучающего курса.


❓ Хочешь проверить, всё ли работает?

Попробуй на чистом Alpine (если установлен):
```shell
docker pull printfchamp/mytest:1.0
docker run printfchamp/mytest:1.0
```

Если работает — образ успешно опубликован.


### Пример работы volume:
✅ Шаг 1: создать контейнер и примонтировать volume
```shell
docker run -d --name withvol -v mydata:/data alpine sleep 1000
```

Здесь происходит:
1. Docker проверяет: существует ли volume mydata
- если нет → создаёт его

2. Запускает контейнер withvol

3. В контейнер монтирует папку /data, которая хранится в volume

Контейнер живёт 1000 секунд — это нужно просто чтобы он не завершился.


✅ Шаг 2: записать данные в volume
```shell
docker exec -it withvol sh -c "echo hello > /data/hello.txt"
```
Мы заходим в контейнер и пишем файл /data/hello.txt.

Но /data — это volume → значит файл сохраняется на хосте!

✅ Шаг 3: удалить контейнер
```shell
docker rm -f withvol
```
❗ Контейнер удалён
✔ Но volume mydata остаётся


✅ Шаг 4: запускаем новый контейнер с тем же volume
```shell
docker run -it --name withvol2 -v mydata:/data alpine cat /data/hello.txt
```
Новый контейнер → новая файловая система
Но volume прежний!

Поэтому файл hello.txt сохраняется и выводится.